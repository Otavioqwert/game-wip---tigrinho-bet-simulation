...
const SkillNode: React.FC<SkillNodeProps> = ({ skill, level, prestigePoints, isAvailable, buySkill }) => {
    ...
    const statusClasses = isMaxLevel 
        ...
        : isAvailable 
            ? (canAfford ? "border-purple-500 bg-purple-500/10 hover:scale-105 shadow-[0_0_10px_rgba(168,85,247,0.2)]" : "border-purple-900 bg-black/40 opacity-80")
        ...
        <div className="relative group z-10 hover:z-50">
            ...
            >
                ...
                <div className={`absolute -top-2 -right-2 w-7 h-7 rounded-full flex items-center justify-center text-xs font-black border-2 shadow-lg
                    ${isMaxLevel ? 'bg-green-600 border-green-400' : 'bg-purple-600 border-purple-400'}`}>
                    {level}
                </div>
                ...
                {!isMaxLevel && isAvailable && (
                     <div className="absolute bottom-1 left-2 right-2 h-1 bg-gray-800 rounded-full overflow-hidden">
                        <div 
                            ...
                            style={{ width: `${Math.min(100, (prestigePoints / (cost || 1)) * 100)}%` }}
                        />
                     </div>
                )}
            </div>
            ...
            <div className="absolute invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all duration-200 bottom-full left-1/2 -translate-x-1/2 mb-3 w-48 p-3 bg-gray-900 border-2 border-purple-500 rounded-lg shadow-2xl pointer-events-none z-[60]">
                <h4 className="font-bold text-purple-300 text-sm border-b border-purple-500/30 pb-1 mb-1">{skill.name}</h4>
                ...
                {!isMaxLevel ? (
                    <div className="flex justify-between items-center mt-1 pt-1 border-t border-white/10">
                        ...
                        <span className={`text-xs font-black ${canAfford ? 'text-yellow-400' : 'text-red-400'}`}>
                            {cost} PA
                        </span>
                    </div>
                ) : (
                    <p className="text-center text-green-400 font-black text-[10px] uppercase">Nível Máximo</p>
                )}
