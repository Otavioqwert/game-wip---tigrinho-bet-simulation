...
const SecondarySkillNode: React.FC<SecondarySkillNodeProps> = ({ skill, level, prestigePoints, bal, isAvailable, buySkill }) => {
    ...
    const costString = skill.costType === 'pa' ? `${cost} PA` : `$${cost.toFixed(2)}`;

    const baseClasses = "relative bg-black/50 p-4 rounded-lg border-2 transition-all duration-300 w-full text-left h-full flex flex-col";
    ...
    const btnClasses = `
        w-full mt-auto py-2 px-3 font-bold text-sm rounded-md transition-all duration-200
    ...
    return (
        <div className={`${baseClasses} ${availabilityClasses}`}>
            <div>
                ...
                <p className="text-sm mt-1 mb-3 min-h-[40px] text-gray-300">{skill.description(level)}</p>
            </div>
            <button
                ...
                `}
            >
                {isMaxLevel ? 'MAX' : `Custo: ${costString}`}
            </button>
        </div>
    );
};
